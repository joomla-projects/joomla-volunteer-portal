...
<form action="<?php echo htmlspecialchars(Uri::getInstance()->toString()); ?>" method="post" name="adminForm"
    ...
    <div class="row-fluid">
        ...
        <div class="page-header">
            <h1><?php echo Text::_('COM_VOLUNTEERS_TITLE_REPORTS') ?><?php if ($this->category) :
                ?>: <?php echo $this->category; ?><?php
                endif; ?></h1>
        </div>
    </div>
        ...
        <?php foreach ($this->items as $i => $item) : ?>
            <div class="row-fluid report">
                ...
                <div class="span10">
                    ...
                    <p><?php echo HtmlHelper::_('string.truncate', strip_tags(trim($item->description)), 500); ?></p>
                    <a href="<?php echo Route::_('index.php?option=com_volunteers&view=report&id=' . $item->id) ?>"
                       class="btn">
                        <span class="icon-chevron-right"></span><?php echo Text::_('COM_VOLUNTEERS_READ_MORE') ?>
                        &nbsp;<?php echo $item->title; ?>
                    </a>
                </div>
            </div>
